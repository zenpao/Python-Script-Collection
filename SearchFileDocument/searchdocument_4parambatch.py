...
def search_files_batch(root, pattern, content, folders=None, batch_size=100):
    ...
    # Split the content input into individual search terms without lowercasing them
...
def contains_content(file_path, search_terms):
    ...
    if file_extension == '.txt':
        encoding = detect_encoding(file_path)  # Use chardet to detect encoding
    ...
    with open(file_path, 'rb') as file:
        ...
        if file_extension == '.pdf':
            with pdfplumber.open(file) as pdf:
                ...
                file_content = pdf_text.lower()
        else:
            file_content = file.read().decode(encoding, errors='ignore').lower()
...
file_pattern = input("Enter the file pattern or extension (e.g., *.txt, *.html, *.pdf): ")
